...
from utils import load_data_with_ewt
...
def train():
    title = "California_summer_20120701-20120707"
    filename = '../../data/week_data/data/California_summer_20120701-20120707新.csv'
    K = 3
    seq_len = 9
    x_trains, y_trains, x_tests, y_tests = load_data_with_ewt(filename, K, seq_len)
    ...
    for k in range(K):
        ...
        predict_ewt_lstm += model.predict(x_tests[k])

    fig, ax = plt.subplots(figsize=(10, 5), layout='constrained')
    ax.set_title(title)
    ax.plot(y_tests[K], label='true')
    ax.plot(predict_ewt_lstm, label='predict')
    ax.grid()
    ax.legend()
    plt.show()

    mse = cal_mse(predict_ewt_lstm, y_tests[K])
    print(f"mse:{mse}")

    df = pd.DataFrame(data={
        'true_value': y_tests[K].reshape(-1),
        'predict': predict_ewt_lstm.reshape(-1),
    })
