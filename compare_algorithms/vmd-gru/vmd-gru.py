...
from utils import load_data_with_vmd
...
def train():
    title = "washingtong_summer_20120701-20120707"
    filename = '../../data/week_data/data/washingtong_summer_20120701-20120707新.csv'
    K = 3
    seq_len = 9
    x_trains, y_trains, x_tests, y_tests = load_data_with_vmd(filename, K, seq_len)
    ...
    for k in range(K):
        ...
        model.fit(x_trains[k], y_trains[k], epochs=200, batch_size=64, validation_split=0.1, verbose=1, callbacks=[callback])
        predict_vmd_gru += model.predict(x_tests[k])

    fig, ax = plt.subplots(figsize=(10, 5), layout='constrained')
    ax.set_title(title)
    ax.plot(y_tests[K], label='true')
    ax.plot(predict_vmd_gru, label='predict')
    ax.grid()
    ax.legend()
    plt.show()

    mse = cal_mse(predict_vmd_gru, y_tests[K])
    print(f"mse:{mse}")

    df = pd.DataFrame(data={
        'true_value': y_tests[K].reshape(-1),
        'predict': predict_vmd_gru.reshape(-1),
    })
